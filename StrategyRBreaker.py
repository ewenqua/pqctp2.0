...
class StrategyRBreaker(threading.Thread, Strategy):
    ...
    def run(self):
        ...
        if holding == 0 and self.tick['LastPrice'] > Bbreak :
            self.PrepareOrder(self.tick['InstrumentID'], D_Buy, OF_Open, self.volume, self.tick['UpperLimitPrice'])
        if holding == 0 and self.tick['LastPrice'] < Sbreak:
            self.PrepareOrder(self.tick['InstrumentID'], D_Sell, OF_Open, self.volume, self.tick['LowerLimitPrice'])
        if holding > 0 and self.tick['LastPrice'] < Senter and self.tick['HighestPrice'] > Ssetup:
            self.PrepareOrder(self.tick['InstrumentID'], D_Sell, OF_Close, self.volume, self.tick['LowerLimitPrice'])
            self.PrepareOrder(self.tick['InstrumentID'], D_Sell, OF_Open, self.volume, self.tick['LowerLimitPrice'])
        if holding < 0 and self.tick['LastPrice'] > Benter and self.tick['LowestPrice'] < Bsetup:
            self.PrepareOrder(self.tick['InstrumentID'], D_Buy, OF_Close, self.volume, self.tick['UpperLimitPrice'])
            self.PrepareOrder(self.tick['InstrumentID'], D_Buy, OF_Open, self.volume, self.tick['UpperLimitPrice'])
